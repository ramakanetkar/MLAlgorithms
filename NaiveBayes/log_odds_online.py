...
test = NaiveBayes('split.train', ['lib', 'con'])

top_con = []
top_lib = []
for w, p in test.P_wv[0].items():
	top_lib.append((w, log(p) - log(test.P_wv[1][w])))

for w, p in test.P_wv[1].items():
	top_con.append((w, log(p) - log(test.P_wv[0][w])))

result = [top_lib, top_con]
for item in result:
	for w, p in islice(sorted(item, key=lambda x: x[1], reverse=True), 20):
		print (w, '%.4f' % p)
	print ()
