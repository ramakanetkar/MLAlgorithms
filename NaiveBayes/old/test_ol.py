...
class NaiveBayes:
	...
	def getP_wv(self):
		for i in range(len(self.Vocabulary)):
			...
			for w in self.allVocabs:
				...
				if w=='it\'s': print('nk: ' + str(self.Vocabulary[i][w]) + 'n: ' + str(n) + 'vocab: ' + str(len(self.allVocabs)) )
	...
	def classify(self, doc):
		...
		for i in range(len(self.Vocabulary)):
			...
			print(sum([log(self.P_wv[i][w]) if w in self.allVocabs else 0 for w in words]))
...
class TestNB:
	def __init__(self, train_file, test_file, V):
...
print('online')
TestNB('split.train', 'split.test_check', ['con', 'lib'])
