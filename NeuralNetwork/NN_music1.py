...
def main():
    ...
    for x in range(10000):
        sgdWeights1, sgdWeights2 = sgd(input_array, output_array, sgdw1, sgdw2, x)
...
def gd(input_array, output_array, gdw1, gdw2):
    ...
    run_count = 0
    while (loss_diff < 0.5 and run_count <=10000):
        ...
        gdw2 += 0.4*np.dot(hidden_layer.T,output_delta)
        gdw1 += 0.4*np.dot(input_layer.T,hidden_layer_delta)
        if run_count == 0:
            ...
            old_loss = new_loss
            new_loss = np.sum(error_sq)/2
            loss_diff = (new_loss - old_loss)/new_loss
        print(new_loss)
        run_count += 1
